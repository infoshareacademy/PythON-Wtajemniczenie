...
def cinema_movies_schedule():
    ...
    if cinema_date == date.today():
        _print_only_future_movies_at_weekday(movies_at_weekday)
    else:
        _print_all_movies_at_weekday(movies_at_weekday)


def _print_only_future_movies_at_weekday(movies_at_weekday):
    current_time = datetime.now().time()
    for movie_showtime in movies_at_weekday:
        if current_time <= movie_showtime.showtime:
            print(f"{movie_showtime.showtime} {movie_showtime.movie}")


def _print_all_movies_at_weekday(movies_at_weekday):
    for movie_showtime in movies_at_weekday:
        print(f"{movie_showtime.showtime} {movie_showtime.movie}")
