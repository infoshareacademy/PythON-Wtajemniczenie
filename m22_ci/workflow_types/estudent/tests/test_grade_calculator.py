...
def test_trivial_policy_doesnt_promote_when_all_grades_are_failing():
    failing_grade = Grade(value=1)
    grades = [failing_grade, failing_grade, failing_grade]
    assert GradeCalculator.trivial_promotion_policy(grades) is False


def test_trivial_policy_promote_when_single_grade_is_passing():
    failing_grade = Grade(value=1)
    passing_grade = Grade(value=2)
    grades = [failing_grade, failing_grade, failing_grade, passing_grade]
    assert GradeCalculator.trivial_promotion_policy(grades) is True
